...
def creer_fen(fen, text_label):
    label = tk.Label(fen, text=text_label)
    button = tk.Button(fen, text="Revenir vers root", command=changer_fen(False, True, fen))
...
def afficherpiece(piece,fenetre,coin,Credit,fontStyle):
    #print(fontStyle)
...
def affichagescore(fenetre,explosion,Resultat,fontStyle):
    #print(fenetre)
...
def affichage_meilleur_score(fenetre,coupe,MeilleurScore,fontStyle): 
...
def affichervie(Nvie1,Nvie2,Nvie3,Vie,fontStyle):
    global fin,ok
...
def vitesse_apparition(vitesseapparition):
    ...
    return vitesseapparition
    #Défini un type de bloc et l'affiche
def create_brick(canvas,BRICK,fenetre):
    ...
    vitesseapparition=vitesse_apparition(vitesseapparition)

    # ATTENTION     #fenetre.after(vitesseapparition,create_brick(canvas,BRICK,fenetre))
...
def descente(BRICK,canvas,fenetre,Nvie1,Nvie2,Nvie3,Vie):
    global bx,by,fin
    if fin!=0:
        nb = -1
    while(nb<len(BRICK)-1):
        if (canvas.coords(BRICK[nb][0])[1]==500):
            ...
            if fin>0:
                fin-=1
            affichervie(Nvie1,Nvie2,Nvie3,Vie)
            lose(fin)
        else:
            canvas.move(BRICK[nb][0],bx,by)
            nb +=1
    vitesse_chute()
    #fenetre.after(vitessechute,descente(BRICK,canvas,fenetre,Nvie1,Nvie2,Nvie3,Vie))
...
def destroy_bloc(shot):
    global Var1,Var2,score,x,by,fenetre
        ...
        while(nb<len(BRICK)-1) and bug==0:
            ...
            if canvas.coords(shot)!=[] :
                ...
                if( canvas.coords(shot)[1]<=canvas.coords(BRICK[nb][0])[3] and canvas.coords(shot)[1]>=canvas.coords(BRICK[nb][0])[1] and canvas.coords(BRICK[nb][0])[0]-20<=midShot and midShot<=canvas.coords(BRICK[nb][0])[2]+20):
                    ...
                    Var2=0
                    score=score+1
                    affichagescore(fenetre,explosion,Resultat)
                    gagnerpiece()
                else:
                    nb+=1
                    Var1 =0
...
def lancer_jeu():
    def clavier(event):
        print("yes")
        global coords, shot, Var1, Var2, gauche, droite, canvas, player
        print(canvas)
        move = event.keysym
    ...
    coin=PhotoImage(master=fenetre,file='image/coin.png')
    coupe=PhotoImage(master=fenetre,file='image/coupe.png')
    explosion=PhotoImage(master=fenetre,file='image/epee.png')
    p=PhotoImage(master=fenetre,file='image/rocket1.png')
    f=PhotoImage(master=fenetre,file='image/coeur.png')
    image=PhotoImage(master=fenetre,file='image/espace5.png')
    titre=PhotoImage(master=fenetre,file='image/titre.png')
    images=PhotoImage(master=fenetre,file='image/espace5.png')
    down=PhotoImage(master=fenetre,file='image/down.png')
    up=PhotoImage(master=fenetre,file='image/up.png')
    bg=PhotoImage(master=fenetre,file='image/espace3.png')
    ...
    affichervie(Nvie1,Nvie2,Nvie3,Vie,fontStyle)
    affichagescore(fenetre,explosion,Resultat,fontStyle)
    afficherpiece(piece,fenetre,coin,Credit,fontStyle)
    affichage_meilleur_score(fenetre,coupe,MeilleurScore,fontStyle)
    ...
    create_brick(canvas,BRICK,fenetre)
    descente(BRICK,canvas,fenetre,Nvie1,Nvie2,Nvie3,Vie)
...
def changer_fen(in_menu, in_a_fen, fen):
    def changer_utilite():
        if in_menu and not in_a_fen:
            menu.withdraw()
            fen.deiconify()
        else:
            fen.withdraw()
            menu.deiconify()
    return changer_utilite


menu = tk.Tk()

fen_jouer = tk.Toplevel(menu)
fen_tuto = tk.Toplevel(menu)
fen_skin = tk.Toplevel(menu)

creer_fen(fen_jouer, "Je suis dans la fenetre a")
creer_fen(fen_tuto, "Je suis dans la fenetre b")
creer_fen(fen_skin, "Je suis dans la fenetre c")

fen_jouer.withdraw()
fen_tuto.withdraw()
fen_skin.withdraw()

fen_jouer_button = tk.Button(menu, command=lancer_jeu, text="Play")
fen_tuto_button = tk.Button(menu, command=changer_fen(True, False, fen_tuto), text="Tuto")
fen_skin_button = tk.Button(menu, command=changer_fen(True, False, fen_skin), text="Skin")

fen_jouer_button.grid()
fen_tuto_button.grid(column=0, row=1)
fen_skin_button.grid(column=0, row=2)
